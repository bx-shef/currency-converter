...
(контекст — `dialogId` + `messageId` выбранного сообщения). Поле ввода чата там недоступно,
поэтому виджет определяет плейсмент через `$b24.placement.placement` и в мобильном контексте
показывает кнопку **«Скопировать»** вместо «Вставить в чат».
...
- [ ] Проверить вручную в реальном мобильном портале (iOS и Android отдельно): пункт меню появляется, слайдер открывается, конвертер и «Скопировать» работают.
