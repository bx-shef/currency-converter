...
- Копирование результата формулы «чистым» числом (`12.34`) — для вставки в Excel
...
- Светлая и тёмная тема — переключатель в шапке (выбор запоминается, при первом
  визите учитывается системная тема)
...
```
app/
  app.vue                 — каркас (шапка, навигация, подвал, SEO, Метрика)
  pages/index.vue         — экран конвертера: строки валют, ввод, прописью, формула
  utils/converter.ts      — конвертация и адаптивный шаг (чистые функции)
  utils/formatters.ts     — формат чисел и формула (чистые функции)
