...
- Актуальные курсы НБ РБ (BYN, RUB, KZT, CNY, TRY, USD, EUR)
...
- Встройка в Битрикс24 — виджет `IM_TEXTAREA` в панели чата с вставкой суммы в сообщение
  и пункт мобильного контекстного меню сообщения `IMMOBILE_CONTEXT_MENU`
...
- **Scopes (права):** `user_brief`, `im`, `placement`, `mobile`

Страница `/install` сама вызовет `placement.bind` и зарегистрирует два места встраивания:
`IM_TEXTAREA` (виджет в панели чата, веб/десктоп) и `IMMOBILE_CONTEXT_MENU` (пункт
контекстного меню сообщения в мобильном приложении Б24, scope `mobile`). Оба используют
один обработчик `/widget/converter`. Мобильную витрину нужно проверять вручную в мобильном
портале; перед модерацией — отметить «Совместимо с мобильной витриной» (issue #89).
...
### Мобильная встройка и модерация витрины (issue #89)

Плейсмент `IMMOBILE_CONTEXT_MENU` открывает виджет в слайдере мобильного приложения Б24
(контекст — `dialogId` + `messageId` выбранного сообщения). В мобильном WebView нет Clipboard
API (копирование молча падает), поэтому виджет определяет плейсмент через
`$b24.placement.placement` и в мобильном контексте **скрывает кнопки копирования в буфер**,
оставляя основное действие **«Вставить в чат»** (прописью) — `im:setImTextareaContent`
работает в обоих плейсментах.

Чеклист перед отправкой на модерацию мобильной витрины:

- [ ] **UI Kit обязателен** — используется `@bitrix24/b24ui-nuxt` (`B24Button`, `B24InputNumber`…).
- [ ] Отметить галку **«Совместимо с мобильной витриной»** в карточке приложения (появляется в кабинете разработчика).
- [ ] Проверить вручную в реальном мобильном портале (iOS и Android отдельно): пункт меню появляется, слайдер открывается, конвертер и «Вставить в чат» работают (кнопок копирования в мобильном контексте быть не должно).
- [ ] Сверить актуальные требования к мобильным приложениям на developer.bitrix24.
