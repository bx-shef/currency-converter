...
версией в разделе `pnpm.overrides` (`package.json`).

**Здесь есть уязвимости, которые доезжают до браузера — в отличие от соседнего
`app-convert-bbocode-md`, где всё сборочное.** `axios` — прямая зависимость
`@bitrix24/b24jssdk` (его HTTP-слой, `abstract-http.mjs`) и **попадает в клиентский
бандл**: проверяется поиском `AxiosError` в `.output/public/_nuxt/*.js` после
`pnpm generate`. Значит advisory на `axios` — реальная поверхность атаки, а не
«только сборка», и чинится в первую очередь. Остальные планки (`tar`, `postcss`,
`svgo`, `esbuild`, `shell-quote`, `js-yaml`, `brace-expansion`, `form-data`) —
инструменты сборки и разработки, в бандл не попадают.

Правила:
- **Скоупить по мажору**, когда в дереве живут несколько веток пакета
  (`"brace-expansion@2": "^2.1.4"`, `"js-yaml@4": "^4.3.1"`). Общая планка на все
  ветки сломает разрешение — разным потребителям нужен разный API.
