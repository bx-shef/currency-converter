...
- Штамп ревью: каждый `.md`-документ несёт строку `> Last reviewed: YYYY-MM-DD` (ISO-дата)
  блок-цитатой сразу под заголовком H1. Дату обновляй **только** когда реально пересмотрел
  содержимое на актуальность — не при косметических правках формата.
