...
- `app/plugins/webVitals.client.ts` + `app/utils/webVitals.ts` — Core Web Vitals (LCP/CLS/INP)
  ...
  PII «shape/outcome»). Plugin — **только standalone** (в iframe Б24 Метрика заглушена → пропускаем
  и не грузим либу), `web-vitals` — динамический импорт (вне критического пути). Чистое ядро
  `webVitalGoal` (маппинг name+rating → имя цели) покрыто `tests/webVitals.test.ts`.
