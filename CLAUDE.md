...
- Штамп ревью документов (`> Last reviewed: YYYY-MM-DD` под H1) и весь рабочий процесс
  (ветки/PR/ревью/CI/мерж) — в [`docs/PROCESS.md`](docs/PROCESS.md). Наличие штампа во всех
  отслеживаемых `.md` проверяет `tests/mdReviewStamp.test.ts` (`reporting-kit/` исключён).
