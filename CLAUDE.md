...
> Точка входа и статус проекта — [`docs/PROJECT_MAP.md`](docs/PROJECT_MAP.md); рабочий процесс
> (ветки/PR/ревью/CI/мерж) — [`docs/PROCESS.md`](docs/PROCESS.md). Этот файл — архитектура и
> конвенции кода.
...
## Безопасность зависимостей (`pnpm.overrides`)

Целевое состояние — `pnpm audit` показывает **0 уязвимостей**. Уязвимые пакеты
приходят транзитивно, обычным бампом их не обновить, поэтому чиним принудительной
версией в разделе `pnpm.overrides` (`package.json`). Текущий состав планок и
причины — в [`docs/PROJECT_MAP.md`](docs/PROJECT_MAP.md) §3.6, здесь только правила.

**Важная особенность этого проекта: часть уязвимостей доезжает до браузера** —
в отличие от соседнего `app-convert-bbocode-md`, где всё сборочное. `axios` —
прямая зависимость `@bitrix24/b24jssdk` (его HTTP-слой, `core/http/abstract-http.mjs`
делает `import axios, { AxiosError } from 'axios'`) и **попадает в клиентский бандл**;
он же тянется опциональным пиром через `@bitrix24/b24ui-nuxt → @vueuse/integrations`
(та же версия). Поэтому advisory на `axios` — реальная поверхность атаки, а не
«только сборка».

Правила:
- **Сначала выяснить, доезжает ли пакет до браузера.** Проверка: `pnpm generate`,
  затем поиск отпечатка пакета в `.output/public/_nuxt/*.js` (для axios —
  `AxiosError`; для `form-data` — `_boundary`/`combined-stream`, их там нет).
  Что доезжает — чиним в первую очередь.
- **Скоупить по ветке версий**, когда в дереве живут несколько (`"brace-expansion@2"`,
  `"js-yaml@4"`, `"esbuild@0.27"`). Общая планка на все ветки сломает разрешение —
  разным потребителям нужен разный API. И не поднимать ветку, которая под advisory
  не попадает (у `esbuild` уязвимы `>=0.27.3 <0.28.1`, поэтому `0.25.x` не трогаем).
- Планка — **временная мера**: когда «родитель» подтянет исправленную версию,
  строку из `overrides` надо убрать.
- После правки `overrides` — обязательно `pnpm install && pnpm audit` **и** полный
  гейт (`pnpm check` + `pnpm generate`): планки затрагивают сборочный конвейер.
- **Если планка меняет версию внутри HTTP-слоя SDK (`axios`) — одного гейта мало.**
  Автотесты сюда не дотягиваются: `useB24` в них замокан (`tests/nuxt/helpers/mockB24.ts`),
  и ни один тест не поднимает настоящий `B24Frame`. А `init()` ходит в портал по-настоящему
  (`app.info`, `profile`, курсы) на каждой странице внутри портала. Поэтому нужен ручной
  прогон в портале — чек-лист [`docs/PROCESS.md`](docs/PROCESS.md) §2.3, пункты
  **К4 · У1–У5 · В1** (то же правило, что и для мажорного бампа `@bitrix24/b24jssdk`).
- Состав планок закреплён тестом: `tests/toolingGuards.test.ts` требует, чтобы каждый
  пакет из `overrides` был описан в карте проекта (иначе появится планка, про которую
  никто не знает, что её пора снимать).
...
- `app/plugins/webVitals.client.ts` + `app/utils/webVitals.ts` — Core Web Vitals (LCP/CLS/INP)
  → цели Метрики `web_vitals_<metric>_<good|ni|poor>` (бакет по рейтингу, **не** сырое значение —
  PII «shape/outcome»). Plugin — **только standalone** (гейт `isEmbedded` — в iframe Б24 Метрика
  заглушена → пропускаем и не грузим либу), `web-vitals` — динамический импорт (вне критического
  пути). Чистое ядро `webVitalGoal` (маппинг name+rating → имя цели) покрыто `tests/webVitals.test.ts`.
- `app/utils/isEmbedded.ts` — общий предикат «внутри iframe» (`window.self !== window.top`,
  fail-closed при cross-origin throw), покрыт `tests/isEmbedded.test.ts`. Используется плагином
  web-vitals; `public/metrika.js` держит свою inline-копию (грузится до бандла, не может импортить).
...
- `app/components/CopyAnnouncer.vue` — невидимый live-регион (`role="status"`,
  `aria-live="polite"`), озвучивающий результат копирования (issue #169): цвет кнопки и
  подмена её `aria-label` до скринридера **не доходят**. Один на страницу, принимает массив
  состояний всех copy-кнопок. Объявляет **изменившееся** состояние, а не первое не-idle —
  иначе ещё горящий `ok` соседней кнопки заглушил бы `err` и озвучил ложный успех; перед
  новым текстом гасит регион (одинаковая строка подряд не объявляется). Тексты — пропсы
  (RU-дефолты для главной, `t()` в виджете).
...
- `app/pages/install.vue` (layout `clear`) — обработчик установки: `init → placement.bind`
  ...
  `buildLangAll` (`app/utils/langAll.ts`, инъекция `t`; при сбое per-locale — фолбэк на текущий
  тайтл, не роняет bind) — покрыт `tests/langAll.test.ts` (issue #86). Биндит только абсолютный
  HANDLER (требует `NUXT_PUBLIC_SITE_URL` в проде).
...
- Штамп ревью документов и весь рабочий процесс (ветки/PR/ревью/CI/мерж, штамп-конвенция) —
  в [`docs/PROCESS.md`](docs/PROCESS.md); наличие штампа проверяет `tests/mdReviewStamp.test.ts`.
