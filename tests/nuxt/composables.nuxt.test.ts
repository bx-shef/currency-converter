...
describe('useNbrbRates', () => {
  ...
  it('normalises a scaled currency (KZT, scale 1000) and leaves rate-less rows blank (#83)', async () => {
    const fetchMock = vi.fn(async () => MOCK_RATES)
    vi.stubGlobal('$fetch', fetchMock)
    ...
    // KZT is published per 1000 units: bynRate = Cur_OfficialRate / Cur_Scale.
